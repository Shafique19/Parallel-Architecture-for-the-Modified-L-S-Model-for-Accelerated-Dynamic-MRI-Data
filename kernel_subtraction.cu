...
void main()
{
	...
	int start_c = clock();
	H_subtraction(y_real_cpu, y_imag_cpu, M_real, M_imag, S_real, S_imag, nx * ny, nt);
	int stop_c = clock();
	...
	// Launch Kernel
	// cudaEventRecord(start_g);
	D_subtraction << <nblocks, nthreads >> > (d_y_real, d_y_imag, d_M_real, d_M_imag, d_S_real, d_S_imag, nx * ny, nt);
