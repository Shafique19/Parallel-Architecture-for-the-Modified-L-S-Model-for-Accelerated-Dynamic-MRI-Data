...
void H_addition(double*, double*, double*, double*, double*, double*, int, int);
// GPU kernel for subtraction
__global__ void D_addition(double*, double*, double*, double*, double*, double*, int, int);
// Start main function
void main()
{
	int nx = 256;
	...
	for (int i = 0; i < (nx * ny * nt); i++)
	{
		sum_m = sum_m + pow(*(M_real + i), exponent) + pow(*(M_imag + i), exponent);
	}
	...
	for (int i = 0; i < (nx * ny * nt); i++)
	{
		sum_s = sum_s + pow(*(S_real + i), exponent) + pow(*(S_imag + i), exponent);
	}
	...
	int start_c = clock();
	H_addition(y_real_cpu, y_imag_cpu, M_real, M_imag, S_real, S_imag, nx * ny, nt);
	int stop_c = clock();
	...
	cudaMalloc((void**)&d_S_real, sizeof(double) * nx * ny * nt);
	cudaMalloc((void**)&d_S_imag, sizeof(double) * nx * ny * nt);
	...
	cudaMemcpy(d_S_real, S_real, sizeof(double) * nx * ny * nt, cudaMemcpyHostToDevice);
	cudaMemcpy(d_S_imag, S_imag, sizeof(double) * nx * ny * nt, cudaMemcpyHostToDevice);
	// kernel configuration
	dim3 nthreads(11, 32);
	dim3 nblocks((int)ceil(nt / 11), (int)ceil(nx * ny / 32));
	// Launch Kernel
	//cudaEventRecord(start_g);
	D_addition << <nblocks, nthreads >> > (d_y_real, d_y_imag, d_M_real, d_M_imag, d_S_real, d_S_imag, nx * ny, nt);
...
// Host Functions Used 
void H_addition(double* y_real, double* y_imag, double* A_real, double* A_imag, double* B_real, double* B_imag, int AB_r, int AB_c)
{
...
__global__ void D_addition(double* y_real, double* y_imag, double* M_real, double* M_imag, double* S_real, double* S_imag, int xy, int nt)
{
